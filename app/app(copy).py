    ...
    def do_function(self):
        ...
        def pesquisa_disciplina():
            ...
            try:
                ...
                print("Resultados encontrados na busca: " + qtde)
                if qtde > 0:
                    resultado = Disciplina.query.filter(
                        Disciplina.nome.like(texto)).all()

                    for x in resultado:
                        resultadoDisciplinas.append([resultado.id, resultado.nome, reusltado.curso])
                        print("Dados disciplinas")
                        listaTurmas = Turma.query.filter_by(idDisciplina=resultado.id).order_by(Turma.curso.asc(), Turma.turma.asc()).all()
                        print("Turmas pesquisadas")
                        for item in listaTurmas:
                            # print(' '.join([item]))
                            resultadoTurmas.append([item.idDisciplina, item.turma, item.docente, item.horario])
